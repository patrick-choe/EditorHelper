...
namespace EditorHelper.Utils {
    public static class PatchRDString {
        ...
        public static class RDStringPatch {
            public static bool Prefix(string key, out bool exists, Dictionary<string, object> parameters,
                out string __result) {
                RDString.Setup();
                exists = false;
                string text = "";
            ...
            public static bool ExistsLocalizedString(string token) =>
                Localization.ExistsLocalizedString(token) || Translations.ContainsKey(token);
