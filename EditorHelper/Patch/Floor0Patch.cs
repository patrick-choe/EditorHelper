...
namespace EditorHelper.Patch {
	...
	internal static class UpdatePatch {
		...
		private static readonly MethodInfo MultiSelectFloors =
			typeof(scnEditor).GetMethod("MultiSelectFloors", AccessTools.all);
		...
		private static void Prefix(scnEditor __instance, ref bool ___refreshBgSprites, ref bool ___refreshDecSprites) {
			...
			if (Input.GetKeyDown(KeyCode.A)) {
				MultiSelectFloors.Invoke(__instance,
					new object[] {
						__instance.customLevel.levelMaker.listFloors.First(),
						__instance.customLevel.levelMaker.listFloors.Last(), true
					});
			}

			if (!Input.GetKeyDown(KeyCode.X) || !__instance.SelectionIsSingle() ||
			    __instance.selectedFloors[0].seqID != 0) {
				return;
			}

			CopyFloor.Invoke(__instance, new object[] {__instance.selectedFloors[0], true, false});

			foreach (var ev in __instance.events.FindAll(x => x.floor == 0).Where(ev =>
				__instance.EventHasBackgroundSprite(ev) || ev.eventType == LevelEventType.AddDecoration)) {
	...
	internal static class OnSelectedFloorChangePatch {
		private static readonly MethodInfo ShowEventPicker =
			typeof(scnEditor).GetMethod("ShowEventPicker", AccessTools.all);

		private static readonly MethodInfo UpdateFloorDirectionButtons =
			typeof(scnEditor).GetMethod("UpdateFloorDirectionButtons", AccessTools.all);
	...
	internal static class AddEventAtSelectedPatch {
		private static bool Prefix(scnEditor __instance, LevelEventType eventType) {
			...
			if (__instance.events.Exists(x =>
				x.eventType == eventType && x.floor == __instance.selectedFloors[0].seqID)) {
			...
			var count = __instance.events
				.FindAll(x => x.eventType == eventType && x.floor == __instance.selectedFloors[0].seqID).Count;
	...
	internal static class PasteEventsPatch {
		...
		private static bool Prefix(scnEditor __instance, scrFloor targetFloor, IReadOnlyList<LevelEvent> eventsList,
			bool overwrite, bool selectAfterward, ref bool ___refreshBgSprites,
			ref bool ___refreshDecSprites) {
			...
			foreach (var ev in eventsList.Where(ev =>
				!EditorConstants.soloTypes.Contains(ev.eventType) ||
				!__instance.events.Exists(x => x.floor == targetFloor.seqID && x.eventType == ev.eventType))) {
				if (__instance.EventHasBackgroundSprite(ev)) {
					___refreshBgSprites = true;
				}
